...
void MPU6050_Reset_FIFO(mpu6050_t *mpu6050) {
    ...
    uint8_t fifo_disable = 0x00;
    I2C_HandleTypeDef* hi2c1 = mpu6050->_I2C;

    HAL_I2C_Mem_Write(hi2c1, MPU6050_ADDR, FIFO_ENABLE, 1, &fifo_disable, 1, 1000);
    ...
    current_value &= (uint8_t)~0x44; // Clear FIFO_RESET and FIFO_EN bits.
    HAL_I2C_Mem_Write(hi2c1, MPU6050_ADDR, USER_CTRL, 1, &current_value, 1, 1000);
}
...
void MPU6050_Get_Current_Sample(mpu6050_t *mpu6050, float *out)
{
    out[0] = mpu6050->accelerometer.Axyz[0];
    out[1] = mpu6050->accelerometer.Axyz[1];
    out[2] = mpu6050->accelerometer.Axyz[2];
    out[3] = mpu6050->temperature;
    out[4] = mpu6050->gyroscope.Gxyz[0];
    out[5] = mpu6050->gyroscope.Gxyz[1];
    out[6] = mpu6050->gyroscope.Gxyz[2];
}
